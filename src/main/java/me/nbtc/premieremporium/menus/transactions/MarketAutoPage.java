...
public class MarketAutoPage extends Page {
    ...
    public int getPageButtonsCount(@Nullable PageView pageView, Player player) {
        return configRepository.getMarketPlaceGui().getConfig().getInt("items-per-page");
    }

    @Override
    public ItemStack nextPageItem(Player player) {
        return ItemBuilder.legacy(Material.ARROW)
                .setDisplay(configRepository.getMPGString("next-page")).build();
    }

    @Override
    public ItemStack previousPageItem(Player player) {
        return ItemBuilder.legacy(Material.ARROW)
                .setDisplay(configRepository.getMPGString("previous-page")).build();
    }
    ...
    public @NotNull MenuTitle getTitle(DataRegistry dataRegistry, Player player) {
        ...
        return MenuTitles.createModern(configRepository.getMPGString("name").replace("{page}", (index+1) + ""));
    }
    ...
    public @NotNull Content getContent(DataRegistry extraData, Player opener, Capacity capacity) {
        ...
        Button btn = Button.clickable(
                ItemBuilder.legacy(Material.valueOf(configRepository.getMPGString("style-item")), 1, (short)0)
                        .setDisplay(configRepository.getMPGString("style-item-name")).build(),
                ButtonClickAction.plain((menu, event) -> event.setCancelled(true))
        );
        String[] stylePattern = configRepository.getMarketPlaceGui().getConfig().getStringList("style-pattern").toArray(new String[0]);
